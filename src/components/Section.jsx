      ...
      <div className="row g-3 g-lg-5 py-5 overflow-x-auto flex-nowrap" style={{ flexWrap: "nowrap" }}>
        {items.slice(0, 10).map((item) => (
          <div key={item.id} className="col-6 col-sm-4 col-md-3 col-lg-2" style={{ minWidth: "150px" }}>
            <Card title={item.title || item.name} image={item.poster_path} year={(item.release_date || item.first_air_date || "").split("-")[0]} rating={item.vote_average.toFixed(1)} link={`/movie/${item.id}`} />
          </div>
        ))}
      </div>
    </div>
